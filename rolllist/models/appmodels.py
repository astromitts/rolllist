...
class RecurringScheduleItem(models.Model, ScheduleItemMixin, BaseModel):
    ...
    def delete_for_day(self, day):
        """ "Delete" an instance of a recurring item's child item for a
            specific day - which actually means set the is_active flag to False
            so that it is hidden from the schedule view, but also prevented from
            being regenerated by the `set_for_day` function
        """
